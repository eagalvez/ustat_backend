...
from Dependencias.utilsf import batch
import numpy as np
import logging
...
if device_name != '/device:GPU:0':
    logging.warning(
        "GPU device not found. Without a GPU the execution of the program will be very slow.")
else:
...
def matrix_dot(ma, mb):
    ...
    m = tf.matmul(X, tf.transpose(a=Y))
...
def cosine_knn(ma, mb, k):
    ...
    m = tf.linalg.matmul(X, tf.transpose(a=Y))
    _, indexes = tf.math.top_k(m, k)
...
def matrix_analogy(ma, mb, mc, mM):
    ...
    nn = tf.matmul(ag, tf.transpose(a=M))
