from Dependencias.utilsf import isInt_float
import numpy as np
import regex
...
import io
import codecs
from Dependencias.utilsf import get_dimensions, get_num_words
import datetime
...
class Embedding(object):
    ...
    def L1_rowwise(self, replace=True):
        if replace:
            normalize(self.vectors, norm='l1', axis=1,
    ...
    def L1_dimensionwwise(self, replace=True):
        if replace:
            normalize(self.vectors, norm='l1', axis=0,
                      copy=False, return_norm=False)
    ...
    def export(self, path, printHeader=True):
        ...
        with open(path, 'w+') as file:
            ...
            for i in range(len(self)):
                print(words[i] + ' ' + ' '.join(['%.6g' %
                                                 x for x in vectors[i]]), file=file)
    ...
    def sentence_to_vector(self, sentence, lower=True):
        ...
        if num_words == 0:
            logging.warning(
                "Any word of the sentence found in the embedding. Sentence: " + str(sentence))
            return None
...
def remove_duplicates(words, vectors):
    seen = set()
    seen_add = seen.add
    duplicate_indexes = [idx for idx, item in enumerate(
        words) if item in seen or seen_add(item)]
...
def from_BIN(path, vocabulary=None):
    with io.open(path, 'rb') as file:
        ...
        if len(words) != vocab_size_aux:
            raise ValueError("Header says that there are {} words, but {} were read".format(
    ...
    for i in range(len(data.index)):
        ...
        if w not in words:
            ...
            if method_vgg == "mean":
                ind = words.index(w)
                vectors[ind] = (
                    vectors[ind] + np.array(data.iloc[i, 1:]).astype(np.float32)) / 2
